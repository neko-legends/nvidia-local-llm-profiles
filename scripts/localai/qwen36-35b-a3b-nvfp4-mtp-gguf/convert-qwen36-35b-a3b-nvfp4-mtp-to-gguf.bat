...
rem ============================================================
rem  Convert NVIDIA Qwen3.6 35B A3B NVFP4 + MTP to GGUF.
rem
rem  Required:
...
rem    set OUTFILE=C:\path\to\qwen3.6-35b-a3b-nvfp4-mtp.gguf
rem ============================================================
set "SCRIPT_DIR=%~dp0"
...
if not defined OUT_DIR set "OUT_DIR=%CHECKOUT_PARENT%\.local-model-cache\nvidia\Qwen3.6-35B-A3B-NVFP4-MTP-GGUF"
if not defined OUTFILE set "OUTFILE=%OUT_DIR%\qwen3.6-35b-a3b-nvfp4-mtp.gguf"
...
if not exist "%SOURCE_MODEL_DIR%\model.safetensors.index.json" (
  ...
  echo.
  echo Download nvidia/Qwen3.6-35B-A3B-NVFP4 into that folder first.
  echo.
  pause
  exit /b 1
)
...
echo   %OUTFILE%
echo.
echo MTP: bundled in the main GGUF by default.
echo.

"%PYTHON_EXE%" %PYTHON_ARGS% "%LLAMA_CPP_SRC%\convert_hf_to_gguf.py" "%SOURCE_MODEL_DIR%" --outfile "%OUTFILE%" --outtype auto --model-name nvidia-Qwen3.6-35B-A3B-NVFP4-MTP
