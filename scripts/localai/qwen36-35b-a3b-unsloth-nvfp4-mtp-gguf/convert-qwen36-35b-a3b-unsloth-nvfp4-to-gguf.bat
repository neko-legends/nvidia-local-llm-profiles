...
rem Convert Unsloth Qwen3.6 35B A3B NVFP4, including its bundled MTP block, to GGUF.
set "SCRIPT_DIR=%~dp0"
for %%I in ("%SCRIPT_DIR%..\..\..\..") do set "CHECKOUT_PARENT=%%~fI"
if not defined SOURCE_MODEL_DIR set "SOURCE_MODEL_DIR=%CHECKOUT_PARENT%\.local-model-cache\unsloth\Qwen3.6-35B-A3B-NVFP4"
if not defined OUT_DIR set "OUT_DIR=%CHECKOUT_PARENT%\.local-model-cache\unsloth\Qwen3.6-35B-A3B-NVFP4-MTP-GGUF"
if not defined OUTFILE set "OUTFILE=%OUT_DIR%\qwen3.6-35b-a3b-unsloth-nvfp4-mtp-gguf.gguf"
...
if not exist "%SOURCE_MODEL_DIR%\model.safetensors.index.json" (
  echo ERROR: source snapshot not found. Run download-qwen36-35b-a3b-unsloth-nvfp4.bat first.
  pause
  exit /b 1
)
...
"%PYTHON_EXE%" %PYTHON_ARGS% "%LLAMA_CPP_SRC%\convert_hf_to_gguf.py" "%SOURCE_MODEL_DIR%" --outfile "%OUTFILE%" --outtype auto --fp8-as-q8 --model-name unsloth-Qwen3.6-35B-A3B-NVFP4-MTP
