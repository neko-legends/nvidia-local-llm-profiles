...
set "BASE_URL=http://127.0.0.1:39187/v1"
set "MODEL_ALIAS=aeon-ornith-1.0-35b-nvfp4"
set "CASE_PREFIX=aeon-ornith-1.0-35b-nvfp4-vllm-ct-nvfp4-ctx256k"
set "GPU_INDEX=0"
set "MAX_TOKENS=1024"
set "RUNS=1"
set "WARMUP_RUNS=0"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "& '%BENCH%' -BaseUrl '%BASE_URL%' -Model '%MODEL_ALIAS%' -CaseName '%CASE_PREFIX%-prompt10k-gen%MAX_TOKENS%' -GpuIndex %GPU_INDEX% -MaxTokens %MAX_TOKENS% -Runs %RUNS% -WarmupRuns %WARMUP_RUNS% -Temperature 0.6 -Seed 1234 -PromptFile '%PROMPT_10K%' -PromptStyle BookContext -TargetPromptTokens 10000"
if errorlevel 1 goto bench_failed

powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "& '%BENCH%' -BaseUrl '%BASE_URL%' -Model '%MODEL_ALIAS%' -CaseName '%CASE_PREFIX%-prompt200k-gen%MAX_TOKENS%' -GpuIndex %GPU_INDEX% -MaxTokens %MAX_TOKENS% -Runs %RUNS% -WarmupRuns %WARMUP_RUNS% -Temperature 0.6 -Seed 1234 -PromptFile '%PROMPT_200K%' -PromptStyle BookContext -TargetPromptTokens 200000"
if errorlevel 1 goto bench_failed
